...
const schema = new Schema<IGridFile, IGridFileModel>(
  {
    /**
 ...
 * @ignore
 */
schema.virtual("model").get(function () {
 ...
 */
schema.static("getBucket", function (this: IGridFileModel): GridFSBucket {
  if (!this.bucket) {
    assert(
      this.collection.collectionName.endsWith(".files"),
      "Collection Name doesn't end with .files",
    );
...
schema.static(
  ...
  async function (query: object): Promise<IGridFile | null> {
 ...
 */
schema.static("deleteById", function (id) {
...
schema.method("getUploadStream", function () {
  const bucket = (this.constructor as IGridFileModel).getBucket();

  const filename = this.filename || `file-${String(this._id)}`;
  // mongoose generates an id
  return bucket.openUploadStreamWithId(this._id, filename, {
...
schema.method("upload", function (stream, callback) {
  return new Promise((resolve, reject) => {
    ...
    uploadStream.on("finish", (file: any) => {
      ...
      const document = (this.model as unknown as IGridFileModel).hydrate(file);
