...
import router from "../route-layout.tsx";
...
import { Lab, LabProgress } from "@clnt/types/lab";
import {
  ArrowLeft,
  BookOpen,
  CheckCircle,
...
import { useLabBuilderStore } from "@clnt/lib/store/lab-builder-store.ts";

export default function LabTestBuildPageRoute() {
  const location = useLocation();
  const { from } = location.state || {};

  const user = useUser();
  const buildLab = useLabBuilderStore((s) => s.buildLab);
  const lab: Lab = buildLab(user.data?.username);
    ...
    await toast.promise(
      new Promise<void>((resolve) => {
        setTimeout(() => {
          ...
          resolve();
        }, 1000); // fake startup delay
      }),
      {
        loading: "Starting lab instance...",
        success: "Lab instance started",
  ...
  const handleSubmitLab = (
    ...
    // Simulate form data prep (no need to actually send it)
    ...
    toast.promise(
      new Promise<void>((resolve) => {
        setTimeout(() => {
          resolve();
        }, 1000); // fake "submission"
      }),
      {
        loading: "Submitting lab...",
        success: "Submitted successfully (test only)!",
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageMeta title="Test Build" description="Test build only lab" />
      {/* Header */}
